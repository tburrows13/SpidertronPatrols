...
local dock_template = table.deepcopy(data.raw[dock_type]["sp-spidertron-dock-80"])
...
if not (data.raw["spider-vehicle"]["spidertron"] and data.raw["spider-vehicle"]["spidertron"].inventory_size == 80) then
  ...
  data.raw[dock_type]["sp-spidertron-dock-80"] = nil
end

local function create_spidertron_dock(inventory_size)
  local name = "sp-spidertron-dock-" .. inventory_size

  if data.raw[dock_type][name] and data.raw[dock_type][name].inventory_size == inventory_size then return end
